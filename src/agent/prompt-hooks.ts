...
function buildSafetyHook(context: PromptHookContext): string {
  ...
  const lines = [
    ...
    'For `message` with `action="send"`, include target as `channelId` (aliases: `to`, `target`) and text as `content` (aliases: `message`, `text`). `send` supports Discord targets, WhatsApp JIDs/phone numbers, and local channels like `tui`.',
