...
export const discordAgentPromptAdapter: ChannelAgentPromptAdapter = {
  messageToolHints: ({ runtimeInfo }) => {
    ...
    const hints: string[] = [
      '- Discord targets: use `channelId` (aliases: `to`, `target`) with a snowflake ID, `<#channelId>`, or `#channel-name` (prefer adding `guildId` for `#channel-name` lookup). WhatsApp sends from here may use a WhatsApp JID or phone number instead.',
      ...
      '- For local Discord or WhatsApp uploads, include `filePath` in `send`; it may be workspace-relative or under `/discord-media-cache`.',
